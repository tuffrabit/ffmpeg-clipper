...
func ClipIntelH264(bodyJson map[string]interface{}) (string, error) {
	commonclipParams, err := getCommonClipParameters(bodyJson)
	if err != nil {
		return "", fmt.Errorf("encoder.ClipIntelH264: could not get common clip params: %w", err)
	}

	encodingPreset, ok := bodyJson["encodingPreset"].(string)
	if !ok {
		return "", errors.New("encoder.ClipIntelH264: could not determine encoding preset")
	}

	qualityTarget, err := getFloat64FromStringInterface(bodyJson["qualityTarget"])
	if err != nil {
		return "", fmt.Errorf("encoder.ClipIntelH264: could not determine qualityTarget: %w", err)
	}
	...
	if err != nil {
		log.Printf("encoder.ClipIntelH264: error running ffmpeg: %v\n", cmdOutput)
	}
