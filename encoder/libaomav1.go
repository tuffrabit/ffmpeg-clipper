...
func ClipLibaomAv1(bodyJson map[string]interface{}) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("encoder.ClipLibaomAv1: could not get common clip params: %w", err)
	}

	qualityTarget, err := getFloat64FromStringInterface(bodyJson["qualityTarget"])
	if err != nil {
		return "", fmt.Errorf("encoder.ClipLibaomAv1: could not determine qualityTarget: %w", err)
	}
