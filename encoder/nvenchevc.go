...
func ClipNvencHevc(bodyJson map[string]interface{}) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("encoder.ClipNvencHevc: could not get common clip params: %w", err)
	}

	encodingPreset, ok := bodyJson["encodingPreset"].(string)
	if !ok {
		return "", errors.New("encoder.ClipNvencHevc: could not determine encoding preset")
	}

	qualityTarget, err := getFloat64FromStringInterface(bodyJson["qualityTarget"])
	if err != nil {
		return "", fmt.Errorf("encoder.ClipNvencHevc: could not determine qualityTarget: %w", err)
	}
